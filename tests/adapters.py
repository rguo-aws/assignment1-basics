...
def run_multihead_self_attention(
    ...
    """
    max_len = in_features.size(-2)
    attention = MultiheadSelfAttention(d_model, num_heads, max_len)
...
def run_multihead_self_attention_with_rope(
    ...
    """
    max_len = in_features.size(-2)
    attention = MultiheadSelfAttention(d_model, num_heads, max_len, True, theta)
    attention.load_state_dict({
        "q_proj.weight": q_proj_weight,
        "k_proj.weight": k_proj_weight,
        "v_proj.weight": v_proj_weight,
    ...
    res = attention(in_features, token_positions)
    return res
...
def run_rope(
    ...
    res = rope.forward(in_query_or_key, token_positions)
    return res
...
def run_transformer_lm(
    ...
    transformer = TransformerLM(d_model=d_model, num_heads=num_heads, d_ff=d_ff, max_seq_len=S, theta=rope_theta,
                                vocab_size=vocab_size, context_length=context_length, num_layer=num_layers)

...
class MultiheadSelfAttention(Module):
    def __init__(self, d_model: int, nums_head: int, max_seq_len: int, apply_rope: bool = False, theta: float = 0.0, device=None, dtype=None) -> None:
        super().__init__()
    ...
    def forward(self, x: torch.Tensor, token_positions: Int[torch.Tensor, "batch seq_len"]| None = None) -> torch.Tensor:
        ...
        q_transformed = rearrange(q, "... sequence_length (h d_k) -> ... h sequence_length d_k", h = self.nums_head, d_k = self.d_k)
        k_transformed = rearrange(k, "... sequence_length (h d_k) -> ... h sequence_length d_k", h = self.nums_head, d_k = self.d_k)
        v_transformed = rearrange(v, "... sequence_length (h d_k) -> ... h sequence_length d_k", h = self.nums_head, d_k = self.d_k)
